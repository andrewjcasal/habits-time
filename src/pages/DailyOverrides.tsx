...
import { Calendar, Save, RotateCcw, ChevronLeft, ChevronRight } from 'lucide-react'
...
const DailyOverrides = () => {
  ...
  const days: DayData[] = Array.from({ length: 5 }, (_, i) => {
    ...
    const dayEssentials = essentials.map(essential => {
      ...
      const override = overrides.find(
        o => o.essential_id === essential.id && isSameDay(new Date(o.date), date)
      )
  ...
  const updateOverride = async (essentialId: string, date: Date, minutes: number) => {
    ...
    try {
      const existingOverride = overrides.find(
        o => o.essential_id === essentialId && isSameDay(new Date(o.date), date)
      )
      ...
      if (existingOverride) {
        ...
        // Update local state
        setOverrides(overrides.map(o => (o.id === existingOverride.id ? { ...o, minutes } : o)))
      } else {
        // Create new override
        const { data, error } = await supabase
          .from('habits_daily_overrides')
  ...
  const handleInputChange = (essentialId: string, date: Date, value: string) => {
  ...
  const getDisplayValue = (essentialId: string, date: Date, currentMinutes: number) => {
    const key = `${essentialId}-${format(date, 'yyyy-MM-dd')}`
    return editingValues[key] !== undefined ? editingValues[key] : currentMinutes
  }
  ...
  const navigateWeek = (direction: 'prev' | 'next') => {
    const newDate = direction === 'prev' ? addDays(startDate, -7) : addDays(startDate, 7)
      ...
      <div className="max-w-6xl mx-auto">
        <div className="mb-8">
          <div className="flex items-center mb-4">
            ...
            <h1 className="text-2xl font-bold text-neutral-900">Daily Overrides</h1>
          </div>
          <p className="text-neutral-600">
            Customize your essential time allocations for specific days. Changes override your
            default settings.
          </p>
        </div>
        ...
        {essentials.length === 0 ? (
          <div className="text-center py-12 bg-white rounded-lg border border-neutral-200">
            ...
            <h3 className="text-lg font-medium text-neutral-900 mb-2">No essentials found</h3>
            <p className="text-neutral-600 mb-4">
              You need to create essentials first before setting daily overrides.
            </p>
            <a
              href="/essentials"
          ...
          <div className="bg-white rounded-lg border border-neutral-200 overflow-hidden">
            ...
            <div className="grid grid-cols-6 gap-0 border-b border-neutral-200">
              ...
              {days.map((day, index) => (
                ...
                >
                  <div className="font-semibold text-neutral-900">{format(day.date, 'EEE')}</div>
                  <div className="text-sm text-neutral-600">{format(day.date, 'MMM d')}</div>
                </div>
              ))}
            </div>
            ...
            {essentials.map(essential => (
              ...
              >
                <div className="p-4 border-r border-neutral-200 bg-neutral-25">
                  <div className="font-medium text-neutral-900">{essential.activity_type_name}</div>
                ...
                {days.map((day, dayIndex) => {
                  const essentialData = day.essentials.find(e => e.essential.id === essential.id)
                  if (!essentialData) return null

                  const key = `${essential.id}-${format(day.date, 'yyyy-MM-dd')}`
                  ...
                  return (
                    <div key={dayIndex} className="p-4 border-r border-neutral-200 last:border-r-0">
                      <div className="space-y-2">
                        <div className="flex items-center space-x-2">
                          <input
                            ...
                            value={displayValue}
                            onChange={e =>
                              handleInputChange(essential.id, day.date, e.target.value)
                            }
                        ...
                        <div className="flex space-x-1">
                          <button
                            onClick={() => updateOverride(essential.id, day.date, displayValue)}
                          ...
                          {hasOverride && (
                            <button
                              onClick={() => resetToDefault(essential.id, day.date)}
                              disabled={isSavingThis}
                        ...
                        {hasOverride && (
                          <div className="text-xs text-primary-600 font-medium">Override</div>
                        )}
                      </div>
                    </div>
                  )
