...
import { Plus, X, Clock, CheckCircle, Trash2 } from 'lucide-react'
...
const Calendar = () => {
      ...
      for (let halfHour = 0; halfHour < 2; halfHour++) {
        ...
        const habitConflicts = habits.filter(habit => {
          ...
          const conflictingMeeting = meetings.find(meeting => {
            ...
            return (
              originalHabitStartInHours < meetingEndInHours &&
              originalHabitEndInHours > meetingStartInHours
            )
          })
  ...
  }, [allTasks, allTasksLoading, tasksScheduled, habitsLoading, getWorkHoursRange])
  ...
  const getCurrentTimeLinePosition = (date: Date) => {
    ...
    // Check if current time is within displayed hours (7am to work hours end)
    if (currentHour < 7 || currentHour >= end) {
      return null
    }
    ...
    const hourIndex = currentHour - 7 // 7am is the first hour
    const minutePercentage = currentMinute / 60
    const totalPosition = (hourIndex + minutePercentage) * 64 // 64px per hour (h-16 = 64px)
  ...
  const handleCompleteTask = async () => {
    ...
    try {
      const { supabase } = await import('../lib/supabase')
      // Extract original task ID from chunk ID if it's a chunk
      ...
      // Refresh the page to reload tasks and reschedule
  ...
  const handleCreateMeeting = async (e: React.FormEvent) => {
    e.preventDefault()
    try {
      const baseDate = selectedTimeSlot
        ...
        {hourSlots.map((hour, hourIndex) => (
          ...
          >
            ...
            {dayColumns.map((column, columnIndex) => {
              ...
              const sessionsInSlot = getSessionsForTimeSlot(hour.time, column.date)
              const tasksInSlot = tasksScheduled ? getTasksForTimeSlot(hour.time, column.date) : []
              ...
              return (
                ...
                >
                  ...
                  {habitsInSlot.map(habit => {
                    ...
                    const habitHeight = habit.duration ? (habit.duration / 60) * 64 : 64 // 64px per hour (h-16 = 64px)
                    const isRescheduled = habit.isRescheduled || false

                    return (
                      <div
                        key={`habit-${habit.id}`}
                        className={`absolute text-xs p-0.5 rounded border-l-2 flex items-start justify-between ${
                          isCompleted
                  ...
                  {meetingsInSlot.map(meeting => {
                    ...
                    const meetingDuration =
                      (meetingEnd.getTime() - meetingStart.getTime()) / (1000 * 60) // minutes
            ...
            return (
              <div
                ...
                style={{
                  ...
                  height: '2px',
                  display: 'grid',
                  gridTemplateColumns: gridCols.replace('80px ', ''), // Remove the time column from grid
                }}
              >
                {/* Render line for each day column */}
      ...
      {showTaskModal && selectedTask && (
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
          <div className="bg-white rounded-lg p-3 w-full max-w-md mx-4">
            ...
            <div className="mb-3">
              <h3 className="font-medium text-neutral-900 mb-1">{selectedTask.title}</h3>
              <p className="text-sm text-neutral-600">
                Project: {selectedTask.project?.name || 'Unknown'}
              </p>
              <p className="text-sm text-neutral-600">Estimated: {selectedTask.estimated_hours}h</p>
            </div>
