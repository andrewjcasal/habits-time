    ...
    if (input.toLowerCase().includes('end interview') || updatedMessages.length >= 15) {
      setTimeout(() => {
        const aiMessage: Message = {
          ...
          sender: 'ai',
          content:
            "Let's wrap up this interview session. I'll now provide you with feedback on your performance.",
    ...
    <div className="flex flex-col h-[75vh] card bg-white overflow-hidden">
          ...
          <div className="flex justify-between items-center mb-2">
            ...
            <button
              onClick={() => {
                ...
                const newPrompts = generateInterviewPrompts(
                  session.type,
      ...
      <div className="p-4 border-t border-neutral-200 bg-white">
        ...
        <div className="mt-2 text-xs text-neutral-500 flex items-center justify-between">
          ...
          {isActive === false && (
            <button onClick={() => onEndSession(evaluation)} className="btn btn-outline btn-sm">
              Close Session
            </button>
          )}
        </div>
...
const generateAiResponse = (
  ...
  const allResponses = [...commonResponses, ...typeSpecificResponses[interviewType]]
...
const generateEvaluation = (interviewType: InterviewType, messages: Message[]) => {
  ...
  const overallScore = Math.round(
    Object.values(scores).reduce((sum: any, score: any) => sum + score, 0) /
  ...
  if (overallScore >= 8) {
    feedback =
      'Excellent performance! You demonstrated strong technical knowledge and communicated your thoughts clearly. Your approach to the problem was well-structured and efficient. A few minor improvements could be made in edge case handling, but overall this was a very strong interview.'
  } else if (overallScore >= 6) {
    feedback =
      'Good performance. You showed solid technical fundamentals and were able to solve the core problem. Your communication was clear, though could be more concise at times. Consider practicing more complex scenarios to improve your problem-solving speed and efficiency.'
  } else {
    feedback =
      'You have a good foundation, but need more practice. Focus on improving your problem analysis and breaking down complex problems into smaller parts. Work on communicating your thought process more clearly and consider edge cases earlier in your solution.'
  }
