...
const NetworkingActionsList: React.FC<NetworkingActionsListProps> = ({
  actions,
}) => {
  // Count intro requests
  const introRequestCount = actions.filter(
    (action) => action.ask_for_intro
  ).length;
      ...
      {actions.length > 0 ? (
        <div className="space-y-2">
          {actions.map((action) => (
            <div
              key={action.id}
              className="border border-neutral-200 rounded-lg px-1 py-0.5"
            >
              ...
              {action.note && (
                <p className="text-sm text-neutral-600 mt-1">{action.note}</p>
              )}
