...
export function usePeople() {
  ...
  const addPerson = async (
    person: Omit<Person, 'id' | 'user_id' | 'created_at' | 'updated_at'>
  ) => {
    try {
      const {
        data: { user },
      ...
      setPeople(prev =>
        [data, ...prev.filter(p => p.id !== data.id)].sort((a, b) => a.name.localeCompare(b.name))
      )
      return data
    } catch (err) {
  ...
  const updatePerson = async (
    id: string,
    updates: Partial<Omit<Person, 'id' | 'user_id' | 'created_at' | 'updated_at'>>
  ) => {
    try {
      const { data, error } = await supabase
        .from('people')
