...
Deno.serve(async (req: Request) => {
  ...
  try {
    ...
    if (!noteContent.trim() || noteContent.trim().length < 20) {
      return new Response(
        JSON.stringify({ message: 'Note content too short for extraction' }),
        { 
          status: 200, 
          headers: { ...corsHeaders, 'Content-Type': 'application/json' } 
        }
...
Return ONLY valid JSON array format, no markdown or extra text:`
    ...
    if (!content) {
      ...
      return new Response(
        JSON.stringify({ message: 'No wins found in note', winsExtracted: 0 }),
        { 
          status: 200, 
          headers: { ...corsHeaders, 'Content-Type': 'application/json' } 
        }
    ...
    if (!Array.isArray(extractedWins) || extractedWins.length === 0) {
      ...
      return new Response(
        JSON.stringify({ message: 'No valid wins found', winsExtracted: 0 }),
        { 
          status: 200, 
          headers: { ...corsHeaders, 'Content-Type': 'application/json' } 
        }
