...
Deno.serve(async (req: Request) => {
  ...
  try {
    if (!todoistApiKey) {
      return new Response(JSON.stringify({ error: 'Todoist API key not configured' }), {
        status: 500,
    ...
    if (!tasksResponse.ok) {
      ...
      return new Response(JSON.stringify({ error: `Todoist API error: ${tasksResponse.status}` }), {
    ...
    const projectLookup = projects.reduce(
      (acc, project) => {
    ...
    const convertPriority = (todoistPriority: number): 'low' | 'medium' | 'high' | 'urgent' => {
      switch (todoistPriority) {
        case 4:
          return 'urgent' // p1 in Todoist
        case 3:
          return 'high' // p2 in Todoist
        case 2:
          return 'medium' // p3 in Todoist
        case 1:
        default:
          return 'low' // p4 in Todoist (default)
      }
    }
    ...
    const todayTasks = processedTasks.filter(task => task.dueDate === today)

    const overdueTasks = processedTasks.filter(
    ...
    if (user) {
      ...
      tasksWithAI = processedTasks.map(task => {
        ...
        if (existingAnalysis) {
          ...
          if (!existingAnalysis.contentHash) {
            console.log(
              `Task ${task.id} has analysis but no content hash - marking for re-analysis`
            )
      ...
      const updateTasksWithAI = (taskList: any[]) =>
        taskList.map(task => ({
          ...task,
